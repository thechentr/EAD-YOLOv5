...
class PatchDetector(torch.nn.Module):
    ...
    def filter(self, xs, bpda=False, shape_completion=False, simple_shape_completion=False,
        ...
        for x in xs:
            ...
            if shape_completion and not simple_shape_completion:
                if self.n_patch > 1:
                    ...
                    mask = ShapeCompletionL1(mask, soft=soft_shape_completion,square_sizes=self.square_sizes)
    ...
    def forward(self,input):
        ...
        output, _, _ = self.filter(input, bpda=False, shape_completion=True, simple_shape_completion=False,
                soft_shape_completion=False, union=False)
