...
import defense_utils.jpeg.utils as utils
...
class rgb_to_ycbcr_jpeg(nn.Module):
    ...
    def __init__(self):
        ...
        self.shift = nn.Parameter(torch.tensor([0.0, 128.0, 128.0]).cuda())
        #
...
class dct_8x8(nn.Module):
    ...
    def __init__(self):
        ...
        self.scale = nn.Parameter(
            torch.from_numpy(np.outer(alpha, alpha) * 0.25).float().cuda()
        )
