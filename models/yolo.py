...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, (Detect, Segment)):
            ...
            # forward = lambda x: self.forward(x)[0] if isinstance(m, Segment) else self.forward(x) #### xlw
            forward = lambda x: self.forward(x)[0] if isinstance(m, Segment) else self.forward(x)[0] #### xlw append
    ...
    def forward(self, x, augment=False, profile=False, visualize=False, policy_features=None):
        if augment:
            return self._forward_augment(x)  # augmented inference, None
        return self._forward_once(x, profile, visualize, policy_features)  # single-scale inference, train
